...
class Observer
{
  ...
  public function CheckHighJump($event, $jump_height) : void
  {
    $tick  = (double)$this->Server->getTick();
    if (($tick - $this->LastMotionTick) > 90)
    {
      ...
      {
        $this->Logger->debug(TextFormat::ESCAPE."$this->Colorized" . "[SAC] > Player $this->PlayerName jumped $jump_height Blocks high!");
	  }
	}
  }
  ...
  public function CheckSpeedFlyGlide($event) : void
  {
    ...
    if (!$this->SACIsOnGround($this->Player))
    {
         ...
         and !in_array(Block::COBWEB              , $this->clipsurroundings ))
      {
        ...
        {
          $jump_from = new Vector3(0.0, $this->y_pos_old, 0.0);
          $jump_to = new Vector3(0.0, $this->y_pos_new, 0.0);
          $jump_height = $jump_from->distance($jump_to);
          $this->CheckHighJump($event, $jump_height);
